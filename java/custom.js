...
function openModal(src) {
    ...
    // Füge alle Bilder in die slides-Array ein
    const images = document.querySelectorAll('.gallery-thumb');
    slides.length = 0; // leere das Array
    images.forEach((img, index) => {
        slides.push(img.src.replace('_thumb', '_large'));
        if (img.src.replace('_thumb', '_large') === src) {
            currentSlide = index;
        }
    });
}
...
function changeSlide(n) {
    ...
    document.getElementById('modal-img').src = slides[currentSlide];
}
